...
export default function ActionsEvento() {
  ...
  const { isAuthenticated, adminSocket, clientSocket } = useAuthContext();
  const {
    setEventoSelected,
    eventoSelected,
    setTiemposEventos,
  ...
  useEffect(() => {
    ...
    if (adminSocket) {
      ...
      adminSocket.emit("tiempos_empresas_salas", (tiemposEmpresasSalas) => {
  ...
  }, [adminSocket, setTiemposEventos, setTiemposEmpresasSalas, clientSocket]);
            ...
            return (
              ...
              >
                <div className="card-title">
                  ...
                  {/* <p>Mie 18, ene 2023</p> */}
                  <p>{parseFechaToFormatDMY(event.inicioEvento)}</p>
                </div>
                <div className="card-times">
                  <h1>
                    {parseFechaToMinutesAndHours(event.inicioEvento) +
                      " - " +
                      parseFechaToMinutesAndHours(event.finEvento)}
                  </h1>
                </div>
                {isAuthenticated && (
